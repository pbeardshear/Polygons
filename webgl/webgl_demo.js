/*
 *
 *	3D map viewer implemented using webgl
 *
 */

define(['lib/glMatrix', 'lib/webgl-utils'], function () {
	// @private variables
	var gl,
		// Rotation matrices
		mvMatrix,
		pMatrix,
		// vertex, fragment shaders are called a 'program'
		shaderPrograms,
		// Buffers
	...
	return {
		...
		_initBuffers: function () {
			...
			for (var i = 0; i < centers.length; i++) {
				...
				for (var j = 0; j < edges.length; j++) {
					...
					currentBuffer = gl.createBuffer();
					gl.bindBuffer(gl.ARRAY_BUFFER, currentBuffer);
					vertices = this._transformPosition(centers[i], edges[j]);
					...
					vertices = this._computeNormals(centers[i], edges[j]);
					gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(vertices), gl.STATIC_DRAW);
		...
		_computeNormals: function (center, edge) {
		...
		init: function (canvas) {
			...
			// Bind handlers
			// TODO: Clean this up
			canvas.onmousedown = util.onMouseDown;
			document.onmouseup = util.onMouseUp;
...
// TODO: Use ARRAY_ELEMENT_BUFFER and build a mesh out of the vertices,
//		 rather than maintain an array of a bunch of vertices
//		 Since the vertices are static, we can do this once and render the whole mesh as one object
//		 This should improve performance fairly significantly
